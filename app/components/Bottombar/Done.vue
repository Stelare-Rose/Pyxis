...
<template>
	...
	<Icon :height='24' :width='24' @click="clicked" @mouseenter="hovered = true" @mouseleave="hovered = false">
		<Check 
			:strokeColor="hovered ? color.text.leaf : '#000'"
		/>
	</Icon>
	<template #popper >
		<section :style="{'background-color': (color.pastel.leaf + '20'), 'max-height': '30%', 'overflow-y': 'scroll', 'padding-top': '8px', 'border-radius': '12px'}">
			<TagsContainer :color='[color.pastel.leaf]' :textColor='[color.text.leaf]' :text="`Done (${itemsDone.length.toString()})`" size="medium" style="margin: 8px 0px 8px 16px" />
